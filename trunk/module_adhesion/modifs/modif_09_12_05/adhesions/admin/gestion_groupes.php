...
/**************************************************************************************************/
/* Gestion des groupes faisant partie de l'association en fonction de l'année de cotisation       */
/**************************************************************************************************/
	...
	// $NB_SESSION qui représente le nombre de session affiché par le module
	// $ANNEE_DEBUT qui représente la date de début de la première saison de cotisation
	...
	//Récupération dans la base des données de tous les groupes ayant adhéré à l'association
	 $sql = 'SELECT 
				ag.`gr_id` , 
				ag.`gr_name` , 
				ag.`gr_description` , 
	...
	while (list ( $gr_id, $gr_name, $gr_description, $gr_web, $gr_irc, $ses_id, $cot_gr_statut, $cot_gr_type, $cot_gr_montant) = mysql_fetch_row ($resultat)) {
		...
		$cotisation_tab[$ses_id]['cot_gr_montant'][$index]=$cot_gr_montant;
  		/*switch ($cot_type){
		    case 1:
		        $cotisation_tab[$ses_id]['cot_type'][$index]= "Chèque";
		        break;
		    case 2:
		        $cotisation_tab[$ses_id]['cot_type'][$index]= "Virement";
		        break;
		    case 3:
		        $cotisation_tab[$ses_id]['cot_type'][$index]= "Espèce";
		        break;
		    case 4:
